...
class MapDrive(MarkerVisualization):
	def __init__(self):
		...
		rospy.Subscriber("/map_metadata", MapMetaData, self.pointCorners)
	...
	def pointCorners(self, map):
		resolution = map.resolution * 2
		startingX = map.origin.position.x * resolution
		startingY = map.origin.position.y * resolution
		rh = (-startingX, -startingY)
		lh = (startingX, -startingY)
		rl = (-startingX, startingY)
		ll = (startingX, startingY)
		
		self.rvizPointReceived(ll)
		self.rvizPointReceived(lh)
		self.rvizPointReceived(rh)
		self.rvizPointReceived(rl)
		self.rvizPointReceived(ll)



	def rvizPointReceived(self, point):
		while self.global_costmap is None or self.local_costmap is None:
			rospy.logerr("No global or local costmap, doing nothing.")
			rospy.sleep(1.5)
		self.lClickPoints.append(point)
		points = [(p[0], p[1]) for p in self.lClickPoints]
		if len(self.lClickPoints) > 2:
			points_x = [p[0] for p in self.lClickPoints]
			points_y = [p[1] for p in self.lClickPoints]
